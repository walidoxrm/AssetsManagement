...
<div class="card">
    <div class="card-body">
<h3 class="card-subtitle mb-2 text-muted">  Ajout de type d'équipement</h3>
</div>
</div>
<br/>
<div class="container">
<label>Type: </label> <input type="text" name="typeAsset">
<br />
...
if (isset($_POST['submit'])) {
    ...
    $query="INSERT INTO type(libelle,active) values ('".$typeToAdd."','".$active."')";
    $result = sqlsrv_query($conn, $query);
    if($result){
        echo '<script language="javascript">';
        echo 'alert("Type ajouté avec succès !");history.back(0)';
        echo '</script>';
        }
}
}
...
elseif ($todo=='manage') {
    ...
    <div class="card">
    <div class="card-body">
<h3 class="card-subtitle mb-2 text-muted"> Gestion de types</h3>
</div>
</div>
<div class="container">
<table class="table">
...
$todelete=$_POST['toDelete'];
    $query="UPDATE type set active=0 where ID = '".$todelete."'";
    $result = sqlsrv_query($conn, $query);
    if($result){
        echo '<script language="javascript">';
        echo 'alert("Type désactivé avec succès !");history.back(0)';
        echo '</script>';
        }
}
...
if (isset($_POST['Activer'])) {
    include('config.php');
$todelete=$_POST['toDelete'];
    $query="UPDATE type set active=1 where ID = '".$todelete."'";
    $result = sqlsrv_query($conn, $query);
    if($result){
        echo '<script language="javascript">';
        echo 'alert("Type activé avec succès !");history.back(0)';
        echo '</script>';
        }
}
